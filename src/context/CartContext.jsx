...
export const CartProvider = ({ children }) => {
  ...
  // Load cart từ unsafeMetadata khi user sẵn sàng
  useEffect(() => {
    const loadCart = () => {
      if (isSignedIn && user) {
        const metadataCart = user.unsafeMetadata?.cart || [];
        console.log('Cart loaded from unsafeMetadata:', metadataCart);
        setCartItem(metadataCart);
      }
    };
    loadCart();
  }, []);

  // Save cart vào unsafeMetadata mỗi khi cartItem thay đổi
  useEffect(() => {
    ...
    const saveCart = async () => {
      try {
        await user.update({
          unsafeMetadata: { ...user.unsafeMetadata, cart: cartItem },
        });
        console.log('Cart saved to unsafeMetadata:', cartItem);
      } catch (err) {
